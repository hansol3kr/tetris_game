...
```
  ...
  [opus]                  [opus]                  [sonnet]                [opus]
        └───────────────────────┴───────────┬───────────┴───────────────────────┘
                                            ▼
              제품팀 product [opus]  ←→  플레이테스트팀 playtest [opus]
              무엇을·어떤 순서로·무엇을 안 할지     플레이어의 목소리 — 만들지 않고 느낀다
              스코프 · 미결 결정 · 릴리스 준비도     (5인 페르소나 패널)
                          (둘 다 → studio-head → 해당 팀)
```

> **만드는 팀 4 + 느끼는 팀 1 + 정하는 팀 1.** 4팀은 산출물을 만들고, playtest는 아무것도 만들지 않으며(Edit 툴 없음),
> product는 **문서만** 만든다(코드 편집 금지). 이 비대칭이 핵심이다 —
> 만든 사람이 자기 게임을 평가하면 항상 통과시키고, 만드는 사람이 스코프를 정하면 항상 다 만들려 한다.
...
| **무엇을·언제·안 할 것** | product | "**어떻게 만드는가**가 아니라 **만들 가치가 있는가·먼저인가·이번엔 뺄 것인가**?" → YES |

> **product vs publishing-growth**: product는 *"이 SKU가 스토어에 등록됐는가"*(선행조건)를 묻고, publishing은 *"얼마에 어떻게 팔까"*(내용)를 답한다.
> **product vs qa-release**: qa는 종료코드로 **판정**하고, product는 그 판정을 **릴리스 준비도 항목**으로 들고 있는다. product는 게이트를 직접 돌려 판정하지 않는다.
> **product vs studio-head**: product는 결정을 **준비**한다(선택지·대가·추천·**결정하지 않을 때의 비용**). 되돌릴 수 없는 결정은 studio-head가 내린다.
...
| `docs/DECISIONS.md`·`ROADMAP.md`·릴리스 준비도·스코프 계약·배송 검증 | **product** | 문서 전용. 코드 편집 금지. ROADMAP은 publishing에서 이관(2026-07-28) |
...
0. **STUDIO HEAD** 요청을 플레이어 경험 언어로 재해석, go/no-go.
0.5. **product** 스코프 계약 작성 — 포함/제외/성공 기준/**자르는 순서**/선행조건. 요청이 여러 개면 분리하고 **작은 것을 인질로 잡지 않는다**(작은 것 먼저 배송). 이 단계를 건너뛰면 한 줄 요청이 다섯 사이클이 된다(2026-07-28 실제 사례). 그 뒤 첫 수를 gameplay에 라우팅(데이터 우선: GameMode 프리셋으로 되나?).
...
8.5. **product** 릴리스 준비도 체크리스트 — 미해결 BLOCKER 0, 게이트 수치 **직접 확인**, 신규 IAP 제품 ID 스토어 등록, ASC 필수 정보(2회 연속 실패한 항목), 상표 감사 0건, 문서 수치 정합, **새 기능의 진입 경로가 실제로 이어지는지**(정의됐다 ≠ 배송됐다). 통과 못 한 항목을 명시해 studio-head에 올린다.
...
- `"PM, 이번 릴리스에 뭐 넣고 뭐 뺄지 정리해줘"` / `"이거 왜 아직 안 됐지"` / `"태그 붙여도 되나"` → **product**
...
- **opus**: product — 자르는 판단과 미결 결정의 대가 산정은 이 스튜디오에서 가장 비싼 실수가 나는 지점이다(한 줄 요청 → 다섯 사이클, 결정 미룸 → 광고 수익 0). 근거 없는 추정을 자신 있게 말하는 PM이 최악이다.
